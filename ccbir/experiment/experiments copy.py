...
from toolz import first, merge_with, valmap, compose, concat, memoize, dissoc
...
class PSFTwinNetExperiment:
    ...
    def benchmark_generative_model(
        ...
        if max_items is None:
            ...
            num_items = min(max_items, dataset_len)
...
class RetrievalExperiment:
    ...
    def _load_queries_and_gallery_data(
        ...
        query_idxs = query_idxs[torch.randperm(
            len(query_idxs),
            generator=generator,
    ...
    def benchmark_ccbir(
        ...
        queries = BatchDict(self.queries_data[:num_queries])
        results = dict(
